...
class Test_UI:
    ...
    def test_auth(self):
        ...
        with allure.step("Проверяем неуспешную авторизацию."):
            try:
                WebDriverWait(self.driver, 10).until(
                    EC.presence_of_element_located((By.ID, "field:input-passwd:hint"))
                )
                assert self.driver.find_element(By.ID, "field:input-passwd:hint").text == "Неверный пароль"# Успешная авторизация
            except TimeoutException:
    ...
    def test_password_recovery(self):
        ...
        with allure.step("Нажимаем 'Не помню пароль'"):
            #self.driver.find_element(By.CSS_SELECTOR,".Link Link_pseudo Link_view_default Link_weight_medium").click()
            button = WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable((By.CSS_SELECTOR, "a.Link.Link_pseudo.Link_view_default.Link_weight_medium"))
            )
            button.click()
        assert self.driver.find_element(By.CSS_SELECTOR, ".Field-label").text == "Введите символы с картинки"
    ...
    def test_search_non_existent_movie(self):
        ...
        with (allure.step("Поиск в ответе нужного результата")):
            try :
                no_movie_name = self.driver.find_elements(By.CSS_SELECTOR, ".textorangebig")
                assert no_movie_name[0].text == "К сожалению, по вашему запросу ничего не найдено..."
            except TimeoutException:
