import allure
import requests
from config.settings import BASE_URL_API, API_TOKEN, YEAR, GENRE, ACTOR_EN, MOVIE_NAME

@allure.feature("Поиск фильмов")
@allure.story("Поиск фильма по названию")
@allure.title(f"Тест: Поиск фильма по названию {MOVIE_NAME}")
@allure.description("Проверяем, что API возвращает фильм с названием 'Дюна' и статус 200.")
def test_search_movie_by_title():
    response = requests.get(f"{BASE_URL_API}/movie/search?query={MOVIE_NAME}", headers=API_TOKEN)
    with allure.step("Проверка статуса ответа"):
        assert response.status_code == 200, f"Ожидался статус 200, но получен {response.status_code}"
    response_title = response.json()["docs"][0]["name"]
    with allure.step("Проверка названия фильма в ответе"):
        assert response_title == MOVIE_NAME, f"Ожидалось название '{MOVIE_NAME}', но получено '{response_title}'"
